...
export const sendConnectionRequest = async (req, res) => {
    try{
        ...
        //Check if user has send more than 20 connection requests in the last 24 hour
        const last24Hours = new Date(Date.now() - 24 * 60 * 60 * 1000)
        const connectionRequests = await Connection.find({from_user_id: userId, created_at: {$gt: last24Hours }})
        if(connectionRequests.length >= 20){
            return res.json({success: false, message: 'You have sent more that 20 connection request in the last 24 hours'})
        }
        ...
        if(!connection){
           const newConnection = await Connection.create({
            ...
            return res.json({success: true, message: 'Connection request send succesfully'})
        }
